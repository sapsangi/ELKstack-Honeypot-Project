...
├── basic-docker-compose-elk.yml    # Template file (TRACKED - Do not put passwords here)
├── basic-docker-compose-opencti.yml # Template file (TRACKED - Do not put passwords here)
├── .env                            # Environment variables (generated by setup.sh)
├── elk/
│   └── docker-compose-elk.yml      # Local config (UNTRACKED - Edit this file)
├── opencti/
│   └── docker-compose-opencti.yml  # Local config (UNTRACKED - Edit this file)
├── logstash-config/
...
1. **Run the setup script:**
   ...
   ./setup.sh
   ```
   *This will check requirements, generate a `.env` file with tokens, and prepare local directories.*

2. **Update passwords in local files:**
   - The setup script copies the `basic-` templates to the `elk/` and `opencti/` directories.
   - **Edit `elk/docker-compose-elk.yml` and `opencti/docker-compose-opencti.yml`** to set your passwords.
   - **Do NOT edit the files in the root directory.**
   - **Crucial:** If your password contains a `$` sign, you must escape it as `$$` (e.g., `pass$word` -> `pass$$word`).

3. **Configure Kibana Service Token (Required):**
   - Start Elasticsearch: `docker compose -f elk/docker-compose-elk.yml up -d elasticsearch`
   - Generate token:
     ```bash
     sudo docker compose -f elk/docker-compose-elk.yml exec elasticsearch bin/elasticsearch-service-tokens create elastic/kibana kibana-token
     ```
   - Update `elk/docker-compose-elk.yml`: Replace `ELASTICSEARCH_USERNAME/PASSWORD` in the `kibana` service with `ELASTICSEARCH_SERVICEACCOUNTTOKEN=<your_token>`.
...
4. **Start the services:**
   ...
   docker compose -f opencti/docker-compose-opencti.yml up -d
   ```
